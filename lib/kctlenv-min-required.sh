...
function kctlenv-min-required() {
  local path="${1:-${KCTLENV_DIR:-.}}";
  ...
  if [[ "${versions}" =~ ([~=!<>]{0,2}[[:blank:]]*)([0-9]+[0-9.]+)[^0-9]*(-[a-z]+[0-9]+)? ]]; then
    ...
    if [[ "${qualifier}" =~ ^!= ]]; then
      ...
      # Probably not an advisable way to choose a kubectl version,
      # but this is the way this functionality works in kubectl:
